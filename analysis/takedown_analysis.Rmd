...
```{r}
file_list <- list.files(path = "~/governemnt_takedown", pattern ="PRPL-Facebook-Government-Report-20?", full.names = TRUE) 
data_list <- list()
...
combined_df <- combined_df |> select(where(~ any(!is.na(.))))
...
#head(combined_df)
#reorder columns
combined_df <- combined_df %>% select(Platform, Date, everything())
...
combined_df <- combined_df |> select(-Content.Restricted)
```


```{r}
combined_df <- combined_df %>% select(Platform, Date,Country, Content.Restrictions, everything())
names(combined_df)
...
combined_df |> 
  mutate(country_code = case_when(
    ...
    TRUE ~ country_code
  )) |> left_join(vdem |> select(country_name, country_text_id, year, v2x_libdem), 
            by = c("country_code" = "country_text_id", "year" = "year")) |>
  select(Country, country_name, country_code, year, everything()) -> combined_df
...
pop_df <- read_csv("API_IT.NET.USER.ZS_DS2_en_csv_v2_3409961.csv", col_names = TRUE, skip=4)
#head(pop_df)
#help(read_csv)

pop_df <- pop_df |> select(c(`Country Name`, `Country Code`) | starts_with('201') | starts_with('202')) |>
  select(!c("2010", "2011", "2012"))
#names(pop_df)
#help(select)
```
...
flat_pop_df <- read_csv("API_SP.POP.TOTL_DS2_en_csv_v2_3401680.csv", col_names = TRUE, skip=4)
head(flat_pop_df)


flat_pop_df <- flat_pop_df |> 
  select(c(`Country Code`) | starts_with('201') | starts_with('202')) |>
  select(!c("2010", "2011", "2012"))
names(flat_pop_df)
...
```{r}
office_df <- read_csv("Local-Offices-1.csv", col_names = TRUE)
head(office_df)

help(rename)
office_df <- office_df |> 
  mutate("has_facebook_office" = `Facebook Office (1-0)`,
         "has_twitter_office" = `Twitter Office (1-0)`,
         "has_google_office" = `Google Office (1-0)` ) |>
  select(-c(`Facebook Office (1-0)`,`Twitter Office (1-0)`,`Google Office (1-0)`))
head(office_df)
...
combined_df |> mutate(Country = Country.x) |> select(-c(Country.y, Country.x)) |>
  select(Country, everything()) -> combined_df
...
gdp_df <- read_csv("API_NY.GDP.MKTP.CD_DS2_en_csv_v2_31795.csv", col_names = TRUE, skip=4)
head(gdp_df)


gdp_df <- gdp_df |> 
  select(c(`Country Name`, `Country Code`)| starts_with('201') | starts_with('202')) |>
  select(!c("2010", "2011", "2012"))
names(gdp_df)
...
names(gdp_df_fixed)
gdp_df_fixed |> select(-`Country Name`) -> gdp_df_fixed
...
```{r}
civic_df <- read_csv("DSP_CY_v6.csv", col_names = TRUE)
head(civic_df)


civic_df <- civic_df |> 
  select(c("country_name", "country_text_id", "year","v2smgovsmmon", "v2smgovfilcap", "v2smarrest"))
names(civic_df)
...
```{r}
regime_df <- read_csv("political-regime.csv", col_names = TRUE)

#regime_df

regime_df <- regime_df |>
  select(c("Entity","Code","Year", "Political regime"))
names(regime_df)
...
combined_df |>
  ...
  )) |> select(-c(country_name.x, Entity)) -> combined_df
...
```{r}
combined_df |> select(-c(`Country Name`, country_name.y )) -> combined_df
...
model_df |> select(c(-starts_with('Court'), -starts_with('Legal'), -starts_with('Emergency'), -starts_with('Subpoena'), -starts_with('Pen'), -starts_with('Title'), -starts_with('Search'), -starts_with('ER'), -starts_with('Preservation'))) -> model_df_cut_cols
...
#fixes column names, TODO is to remove irrelevant columns first and then to remove values that have NAs so that stepwise AIC will run properly
```{r}
...
C:/Users/Joe/Documents/governemnt_takedown/takedown_analysis.Rmd
